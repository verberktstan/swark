...
(defn key-by
  ...
  `(key-by :id [{:id 12} {:id 34}]) => {12 {:id 12} 34 {:id 34}}`"
   :ret ::spec/key-by-result
   :fn (s/or :1arg ::spec/key-by-1arg
             :2arg ::spec/key-by-2arg
             :3arg ::spec/key-by-3arg)}
  ([f]
   {:pre [(ifn? f)]}
      ...
      (fn [acc k v]
        (if-let [predicate (get spec k)]
          (let [{res ::result :as result} (check predicate v)]
            (cond-> acc (not res) (assoc k result)))
          acc))
      nil
      input)))
