...
(def USER1 {:id 1 :username "Arnold"})
(def USER2 #:user{:id 2 :name "Arnold" :city "Birmingham"})

(t/deftest serialize
  (t/testing "Returns rows for every map-entry"
    (t/are [result props input] (= result (sut/serialize props input))
      [["tx1" "id" "1" "username" "Arnold" ""]]
      {:primary/key :id :transaction/id "tx1" :val/parsers {:id swark/->str}}
      USER1

      [["tx1" "user/id" "2" "user/name" "Arnold" ""]
       ["tx1" "user/id" "2" "user/city" "Birmingham" ""]]
      {:primary/key :user/id :transaction/id "tx1" :val/parsers {:user/id swark/->str}}
      USER2

      [["tx1" "id" "1" "username" "Arnold" "[swark.eav/deleted]"]]
      {:primary/key :id :transaction/id "tx1" :val/parsers {:id swark/->str} :row/flags #{::sut/deleted}}
      USER1)))
